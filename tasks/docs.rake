...
namespace :docs do
    ...
    task :local => [:generate, :publish_local]
    ...
    task :publish_local do
        ...
        puts "📖  💁️     Commiting and pushing the generated documentation…"
        publish_docs_local(tmp_dir, repo_dir, branch, docs_path, git_head_hash(repo_dir))
    ...
    def publish_docs(tmp_dir, repo_dir, branch, docs_dir, for_commit)
        ...
        # Set up remote url
        execute_git(repo_dir, 'remote', 'set-url', '--push', 'origin', 'https://${GITHUB_TOKEN}@github.com/tevelee/Funky.git')
    ...
    def publish_docs_local(tmp_dir, repo_dir, branch, docs_dir, for_commit)
        # Remove all files in the repo, otherwise we might get lingering files that aren’t
        # generated by jazzy anymore. This won’t remove any dotfiles, which is intentional.
        FileUtils.rm_rf(Dir.glob("#{repo_dir}/*"), secure: true)
