...
#import <LTFunctional/NSDictionary+LTUtilities.h>

@interface LTDictionaryUtilitiesTests : XCTestCase

@property (nonatomic, strong) NSDictionary<NSString*, NSNumber*>* items;

@end

@implementation LTDictionaryUtilitiesTests
...
- (void)test_map {
    NSDictionary* mapped = [self.items map:^LTPair*(NSString* key, NSNumber* value) {
        return [LTPair pairWithKey:key value:key];
    }];
    NSDictionary* expected = @{@"a": @"a", @"b": @"b", @"c": @"c"};
    XCTAssertEqualObjects(mapped, expected);
    
    mapped = [self.items map:^LTPair*(NSString* key, NSNumber* value) {
        return [LTPair pairWithKey:@"a" value:value];
    }];
